...
void CPU_Proximity_AnalyzeReceivePacket(Packet *receivePacket, Packet *sendPacket)
{
    ...
    case PROXIMITY_CPU_SENDRATS: //0x30 RATS
        ...
        status = pcd_rats_a(0, MH523.RATS,&MH523.RATSLength);	//发送RATS
        if(status == MI_OK)
        {
            for(i=0; i<MH523.RATSLength; i++)
            {
                sendPacket->ATS[i]	= MH523.RATS[i];
            }
            sendPacket->length		= MH523.RATSLength+3;
            sendPacket->ATSLength	= MH523.RATSLength-2;
            sendPacket->p			= 'Y';
            break;
        }
