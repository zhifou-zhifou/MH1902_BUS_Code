...
typedef struct	//通讯包结构体
{
    ...
    ////////////////////////金融读卡器协议///////////////////////
    unsigned char head;//数据头 STX
    unsigned char sn;//序列号
    unsigned char unuse[4];//厂商预留
    unsigned char command;//指令
    unsigned int length;//数据长度
    unsigned char data[400];//数据域
    unsigned char tail;//数据结尾
    unsigned char crc16;//crc校验
    unsigned char  packet_buf[400];//完整的一包数据
    unsigned int   packrt_buf_len;//完整的一包数据长度
    /////////////////////////////////////////////////////////
